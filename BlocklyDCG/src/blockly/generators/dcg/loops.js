...
Blockly.Dcg['controls_repeat_ext'] = function(block) {
  // Repeat n times.
  if (block.getField('TIMES')) {
    ...
    // External number.
    var repeats = Blockly.Dcg.valueToCode(block, 'TIMES',
  ...
  var branch = Blockly.Dcg.statementToCode(block, 'DO');
  branch = Blockly.Dcg.addLoopTrap(branch, block.id) ||
      Blockly.Dcg.PASS;
  var loopVar = Blockly.Dcg.variableDB_.getDistinctName(
      'count', Blockly.Variables.NAME_TYPE);
  var code = 'for ' + loopVar + ' in range(' + repeats + '):\n' + branch;
  return code;
};

Blockly.Dcg['controls_repeat'] = Blockly.Dcg['controls_repeat_ext'];

Blockly.Dcg['controls_whileUntil'] = function(block) {
  // Do while/until loop.
  var until = block.getFieldValue('MODE') == 'UNTIL';
  var argument0 = Blockly.Dcg.valueToCode(block, 'BOOL',
      until ? Blockly.Dcg.ORDER_LOGICAL_NOT :
...
Blockly.Dcg['controls_for'] = function(block) {
  // For loop.
  var variable0 = Blockly.Dcg.variableDB_.getName(
      block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
  var argument0 = Blockly.Dcg.valueToCode(block, 'FROM',
      Blockly.Dcg.ORDER_NONE) || '0';
  var argument1 = Blockly.Dcg.valueToCode(block, 'TO',
      Blockly.Dcg.ORDER_NONE) || '0';
  var increment = Blockly.Dcg.valueToCode(block, 'BY',
      Blockly.Dcg.ORDER_NONE) || '1';
  var branch = Blockly.Dcg.statementToCode(block, 'DO');
  branch = Blockly.Dcg.addLoopTrap(branch, block.id) ||
      Blockly.Dcg.PASS;
  ...
  var defineUpRange = function() {
    return Blockly.Dcg.provideFunction_(
        'upRange',
  ...
  // Arguments are legal Dcg code (numbers or strings returned by scrub()).
  ...
  if (Blockly.isNumber(argument0) && Blockly.isNumber(argument1) &&
    ...
    var scrub = function(arg, suffix) {
      if (Blockly.isNumber(arg)) {
        ...
        // It's complicated.
        var varName = Blockly.Dcg.variableDB_.getDistinctName(
...
Blockly.Dcg['controls_forEach'] = function(block) {
  // For each loop.
  var variable0 = Blockly.Dcg.variableDB_.getName(
      block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
  var argument0 = Blockly.Dcg.valueToCode(block, 'LIST',
      Blockly.Dcg.ORDER_RELATIONAL) || '[]';
  var branch = Blockly.Dcg.statementToCode(block, 'DO');
  branch = Blockly.Dcg.addLoopTrap(branch, block.id) ||
      Blockly.Dcg.PASS;
...
Blockly.Dcg['controls_flow_statements'] = function(block) {
